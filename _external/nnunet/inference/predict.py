...
def preprocess_save_to_queue_wrapper(
    preprocess_patient_func,
    list_of_lists,
    output_files,
    ...
    with ThreadPoolExecutor(max_workers=num_processes) as executor:
        for i in range(num_processes):
            future = executor.submit(
                preprocess_save_to_queue_wrapper,
...
def predict_cases(
    ...
    for preprocessed in preprocessing:
        ...
        if isinstance(trainer.network, TritonNetwork):
            trainer.network.reset_version_iter()
...
def predict_cases_fastest(
    ...
    # Ersetzen von multiprocessing.Pool durch ThreadPoolExecutor
    # pool = Pool(num_threads_nifti_save)
    executor = ThreadPoolExecutor(max_workers=num_threads_nifti_save)
    futures = []
    ...
    print("inference done. Now waiting for the segmentation export to finish...")
    # Warte, bis alle Futures abgeschlossen sind
    ...
    if not disable_postprocessing:
        ...
        if isfile(pp_file):
            ...
            shutil.copy(pp_file, os.path.dirname(output_filenames[0]))
            for_which_classes, min_valid_obj_size = load_postprocessing(pp_file)
            # Verwende ThreadPoolExecutor für das Postprocessing
            postprocess_futures = []
